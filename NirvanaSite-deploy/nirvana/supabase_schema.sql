...
CREATE INDEX IF NOT EXISTS page_metadata_updated_at_idx ON page_metadata(updated_at DESC);

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_trigger WHERE tgname = 'page_metadata_set_updated_at'
    ) THEN
...
BEGIN
    ...
    ELSIF req.entity_type = 'blog' THEN
        ...
        END IF;
    ELSIF req.entity_type = 'page_metadata' THEN
        IF req.action IN ('create', 'update') THEN
            INSERT INTO public.page_metadata (
                ...
                NULLIF(req.payload->>'open_graph_title', ''),
                NULLIF(req.payload->>'open_graph_description', ''),
                NULLIF(req.payload->>'open_graph_image', ''),
                NULLIF(req.payload->>'twitter_title', ''),
                NULLIF(req.payload->>'twitter_description', ''),
                NULLIF(req.payload->>'twitter_image', ''),
                COALESCE((req.payload->>'noindex')::boolean, false),
                COALESCE((req.payload->>'follow')::boolean, true),
                req.submitted_by,
                NOW()
            )
...
-- Focused approval entry point used by the page metadata editor. Keeping this
-- small also makes the feature safe to deploy as an incremental migration.
CREATE OR REPLACE FUNCTION apply_page_metadata_approval(
    p_request_id UUID,
    p_new_status TEXT DEFAULT 'approved',
    p_comment TEXT DEFAULT NULL
)
...
GRANT EXECUTE ON FUNCTION apply_page_metadata_approval(UUID, TEXT, TEXT) TO authenticated;
...
DO $$
BEGIN
    DROP POLICY IF EXISTS "Published page metadata is viewable by everyone" ON page_metadata;
    CREATE POLICY "Published page metadata is viewable by everyone"
    ON page_metadata FOR SELECT USING (true);
END;
$$;

GRANT SELECT ON TABLE page_metadata TO anon, authenticated;
